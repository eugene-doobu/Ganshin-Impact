...
namespace GanShin.SceneManagement
{
    public class SimpleDemoScene : SpaceScene
    {
        ...
        private void ProcessGlobalUIDebugInput()
        {
            ...
            if (Input.GetKeyDown("2"))
                UIManager.SetPopupOkCancel("타이틀", "메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. 메시지인데 짱김. ", () => { Debug.Log("OK"); }, () => { Debug.Log("Cancel"); });
            
            if (Input.GetKeyDown("3"))
                UIManager.SetToast("테스트1", "퀸카 암핫", EToastType.DEFAULT);

            if (Input.GetKeyDown("4"))
                UIManager.SetToast("테스트2", "그냥 살다 보니 어느새 난 여기 사실 별거 없지 잘은 해 밥벌이", EToastType.NOTIFICATION);
            
            if (Input.GetKeyDown("5"))                
                UIManager.SetToast("테스트3", "Don't ever say it's over if I'm breathin, Racin' to the moonlight and I'm speedin, I'm headed to the stars, ready to go far, I'm star walkin'", EToastType.WARNING);
        ...
        private void ProcessDialogue()
        {
            ...
            if (Input.GetKeyDown("3"))
            {
                ...
                {
                    name             = "Riko",
                    content          = "밥을 어디 옷을 천하를 불어 품으며, 소담스러운 쓸쓸하랴? 사라지지 하는 인생에 이것이다. 더운지라 열락의 실현에 안고, 그들은 우리 불러 그와 가슴이 것이다. 위하여서 인생의 물방아 끓는다. 생명을 수 소금이라 거친 몸이 그림자는 타오르고 놀이 봄바람이다. 품으며, 구하기 내는 인생에 없는 때에, 실로 기쁘며, 인간은 사막이다.",
